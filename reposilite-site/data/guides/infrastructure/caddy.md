...
* If Ingot is running on an other machine, replace `localhost` in the `reverse_proxy localhost:{args.0}` statement to the IP or host of that machine.
* Here the `domain.com` refers to the domain name in used. Typically this needs to be changed.
* The port of the Ingot instance here is 8080, this needs to match your configuration.
...
The caddy reverse proxy can be configured to take care of SSL certificates and server Ingot over HTTPS. 
