...
Ingot supports S3-compatible storages as a backend for Maven repository.
...
By default, Ingot writes objects to the **root** of the bucket, which is why each S3 repository
...
* **Key Prefix** (`prefix`): an optional prefix prepended to every object key. Use it to scope
  Ingot's data within a bucket shared with other tools, e.g. set it to `reposilite` so all objects
  live under `reposilite/…` next to another service's `registry/…`.
* **Shared Bucket** (`sharedBucket`): enable when several Ingot repositories share one bucket.
...
same, or a nested, key namespace, Ingot refuses to load the colliding ones and logs an error on startup.
...
versus an explicit AWS URL), Ingot cannot tell they are the same bucket, so keep the endpoint
...
A lot of Ingot users are using this solution, so here are some tips:

Ingot uses official AWS SDK that uses the latest version of S3 API.
...
so you may want to disable automated bucket creation in Ingot to grant more adequate permissions.
...
* Object listing is not supported - Ingot won't be able to e.g. list all versions of the artifact or display directory content in the UI
